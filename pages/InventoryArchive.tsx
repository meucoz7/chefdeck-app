...
const InventoryArchive: React.FC = () => {
    ...
    const loadArchives = async () => {
        ...
        try {
            ...
            // We only show finalized cycles here
    ...
    const exportCycle = (cycle: InventoryCycle) => {
        ...
        XLSX.writeFile(wb, `Inv_Archive_${new Date(cycle.date).toISOString().split('T')[0]}.xlsx`);
        addToast("Файл сформирован", "success");
    };
            ...
            <div className="pt-safe-top px-5 pb-4 sticky top-0 z-40 bg-[#f2f4f7]/95 dark:bg-[#0f1115]/95 backdrop-blur-md border-b border-gray-100 dark:border-white/5">
                <div className="flex items-center gap-3 pt-4">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Хранение 3 месяца</p>
                    </div>
                </div>
            </div>
            ...
            <div className="px-5 pt-6 space-y-8">
                {groupedArchives.length === 0 ? (
                    <div className="text-center py-20 opacity-50 flex flex-col items-center">
                        ...
                        <p className="text-xs text-gray-400 mt-2 px-10">Завершенные инвентаризации появятся здесь</p>
                    </div>
                ) : (
                    groupedArchives.map(([month, monthCycles]) => (
                        <div key={month} className="space-y-4">
                            <h3 className="text-xs font-black text-purple-500 uppercase tracking-[0.2em] ml-2">{month}</h3>
                            <div className="grid gap-3">
                                {monthCycles.map(c => (
                                    <div key={c.id} className="bg-white dark:bg-[#1e1e24] p-4 rounded-[2rem] shadow-sm border border-gray-100 dark:border-white/5 flex items-center justify-between group">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-2xl bg-purple-50 dark:bg-purple-500/10 flex items-center justify-center text-2xl">📊</div>
                                            <div>
                                                <h4 className="font-bold text-gray-900 dark:text-white text-sm">Инвента {new Date(c.date).toLocaleDateString('ru-RU')}</h4>
                                                <p className="text-[9px] text-gray-400 font-black uppercase">{c.sheets.length} станций • {c.createdBy}</p>
                                            </div>
                                        </div>
                                        <div className="flex gap-2">
                                            <button onClick={() => setSelectedCycle(c)} className="w-9 h-9 rounded-xl bg-gray-50 dark:bg-white/5 flex items-center justify-center text-gray-400 hover:text-purple-500 transition active:scale-90"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg></button>
                                            <button onClick={() => exportCycle(c)} className="w-9 h-9 rounded-xl bg-gray-50 dark:bg-white/5 flex items-center justify-center text-gray-400 hover:text-green-500 transition active:scale-90"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-md animate-fade-in p-4">
                    <div className="bg-white dark:bg-[#1e1e24] w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col h-[85vh]">
                        <div className="p-6 border-b border-gray-100 dark:border-white/5 flex justify-between items-center bg-gray-50/50 dark:bg-black/20">
                            <div>
                                <h2 className="text-xl font-black dark:text-white leading-none">Архив: {new Date(selectedCycle.date).toLocaleDateString()}</h2>
                                <p className="text-[9px] text-gray-400 font-bold uppercase mt-2">Автор: {selectedCycle.createdBy}</p>
                            </div>
                            ...
                            {detailMode === 'summary' ? (
                                <table className="w-full text-left border-collapse">
                                    <thead className="text-[8px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-white/5">
                                        <tr><th className="pb-4">Товар</th><th className="pb-4 text-right">Факт</th></tr>
                                    </thead>
