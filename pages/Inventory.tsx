...
const Inventory: React.FC = () => {
    ...
    const exportConsolidated = () => {
        ...
        const data = consolidatedData.map(d => ({
            "Наименование": d.name,
            "Ед.изм": d.unit,
            "Итого факт": String(d.totalActual).replace(',', '.')
        }));
                        ...
                        {isAdmin && (
                            <div className={`grid ${activeCycle ? 'grid-cols-3' : 'grid-cols-1'} gap-2.5 mb-2`}>
                                <div onClick={() => document.getElementById('xl-import')?.click()} className="col-span-1 bg-sky-100 dark:bg-sky-500/20 rounded-2xl p-2 text-sky-600 dark:text-sky-400 flex flex-col items-center justify-center gap-1 h-20 cursor-pointer active:scale-[0.98] transition-transform group">
                                    ...
                                    <div className="w-8 h-8 rounded-full bg-white dark:bg-black/20 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                                    ...
                                    <h3 className="font-bold text-[9px] uppercase tracking-tighter">Импорт</h3>
                                </div>
                                {activeCycle && (
                                    <>
                                        <div onClick={() => setViewMode('admin')} className="col-span-1 bg-indigo-100 dark:bg-indigo-500/20 rounded-2xl p-2 text-indigo-600 dark:text-indigo-400 flex flex-col items-center justify-center gap-1 h-20 cursor-pointer active:scale-[0.98] transition-transform group">
                                            <div className="w-8 h-8 rounded-full bg-white dark:bg-black/20 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0m-9.75 0h9.75" /></svg>
                                            </div>
                                            <h3 className="font-bold text-[9px] uppercase tracking-tighter">Сводная</h3>
                                        </div>
                                        <div onClick={() => setViewMode('manage')} className="col-span-1 bg-slate-100 dark:bg-white/5 rounded-2xl p-2 text-slate-600 dark:text-gray-300 flex flex-col items-center justify-center gap-1 h-20 cursor-pointer active:scale-[0.98] transition-transform group">
                                            <div className="w-8 h-8 rounded-full bg-white dark:bg-black/20 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4"><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12a7.5 7.5 0 1115 0 7.5 7.5 0 01-15 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.24-.438.613-.431.992a6.759 6.759 0 010 .255c-.007.378.138.75.43.99l1.005.828c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.57 6.57 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.28c-.09.543-.56.941-1.11.941h-2.594c-.55 0-1.02-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.992a6.932 6.932 0 010-.255c.007-.378-.138-.75-.43-.99l-1.004-.828a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.087.22-.128.332-.183.582-.495.644-.869l.214-1.281z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                                            </div>
                                            <h3 className="font-bold text-[9px] uppercase tracking-tighter">Настройки</h3>
                                        </div>
                                    </>
                                )}
                            </div>
