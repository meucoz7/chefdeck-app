...
const InventoryItemRow = React.memo<{
  ...
  const lastParentVal = useRef(item.actual?.toString() || '');
...
const Inventory: React.FC = () => {
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (resGlobal) {
        ...
        scopedStorage.setJson('global_items_cache', globalData);
      }
    } catch (e) {
  ...
  const startInventory = async () => {
    ...
    try {
      ...
      if (data.success) { addToast("Бланк взят в работу", "success"); loadData(true); }
              ...
              if (viewMode === 'filling') {
                if (activeCycle) {
                  ...
                  if (s) {
                    ...
                    setActiveCycle(updated);
                    apiFetch('/api/inventory/cycle', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updated) });
                    setInitialAmount(''); setSelectedGlobalIds(new Set());
